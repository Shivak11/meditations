...
Finish the substantive reflection before styling it. Identify a consequential decision, the evidence behind a correction, a concept worth understanding, a limit and one experiment for next time. Do not invent content to fill a design.

Choose an explanation suited to the actual subject. A before-and-after comparison can show why a correction mattered. A causal diagram can show how a changed input affected the result. A decision example can let the reader change one assumption before seeing the consequence. Keep the same objects visible across changes so the reader can follow the explanation.

Use the user's work when it is available and authorised. Label a constructed example as an illustration. Do not present invented chat excerpts as a historical conversation or claim that an example was executed unless it was tested.
...
- Start with a plain headline and a short explanation, then show the work or worked example. Avoid eyebrows, repeated labels and decorative framework names.
...
- Explain one counterexample clearly and offer one small experiment for later work. Put uncertainty near the claim it qualifies in ordinary language.
...
Keep source ledgers, design records, critique, prompts, raw transcripts and internal record-state labels such as `NO RECORD` outside the final HTML. Put the record verdict in the conversational handoff. Private study creation is authorised output. Saving a lasting lesson, updating memory or sharing the study is a separate action governed by the user's permission.

## Verify and deliver

When a browser is available, inspect desktop and 390px layouts and exercise the meaningful interactions with keyboard and pointer. Check for overflow, clipping, small type, missing labels and confusing feedback. Test the no-JavaScript and print versions. Check computed examples against actual results and ensure the final file makes no remote requests.
...
Deliver a clickable file or download link, the main lesson, the next experiment and the lasting-record verdict. Do not repeat the full study in chat. The absence of StyleRef does not require the user's attention when the bundled fallback succeeds.
